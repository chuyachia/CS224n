...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentVector= np.array([wordVectors[tokens[w]] for w in sentence])
    sentVector = np.mean(sentVector,0)
...
def chooseBestModel(results):
    ...
    ### YOUR CODE HERE
    val_acc = np.array([x["dev"] for x in results])
    bestResult = results[np.argmax(val_acc)]
    #raise NotImplementedError
    ### END YOUR CODE
