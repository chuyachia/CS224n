...
class SequencePredictor(Model):
    ...
    def add_loss_op(self, preds):
        ...
        total_loss = tf.nn.l2_loss(y-preds)
    ...
    def add_training_op(self, loss):
        ...
        gradients = optimizer.compute_gradients(loss) # optimizer.compute_gradients returns a list of (gradient, variable) pairs
        [grad, var] = zip(*gradients) # separate gradient and variable to cut gradient
        if self.config.clip_gradients:            
            grad,self.grad_norm = tf.clip_by_global_norm(grad,self.config.max_grad_norm)
            gradients = zip(grad, var) # regroup gradient and variable to passe to optimizer.apply_gradients
        else:
