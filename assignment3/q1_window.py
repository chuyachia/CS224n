...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(window_size,len(sentence)-window_size):
            w_features = [f for w in sentence[i-window_size:i+window_size+1] for f in w]
            w_label = labels[i-window_size]
            windowed_data.append((w_features,w_label))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedd_matrix = tf.Variable(self.pretrained_embeddings,dtype=tf.float32)
        embeddings = tf.nn.embedding_lookup(embedd_matrix,self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable(name= 'b1',initializer= tf.zeros(shape=[1,self.config.hidden_size]))
        ...
        b2 = tf.get_variable(name= 'b2',initializer= tf.zeros(shape=[1,self.config.n_classes]))
    ...
    def add_loss_op(self, pred):
        ...
        loss_temp = tf.nn.sparse_softmax_cross_entropy_with_logits(labels= self.labels_placeholder,logits= pred)
